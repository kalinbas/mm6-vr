...
MAP_FILE_NAME = "d01.blv"
MAP_KEY = "d01"
MAP_DISPLAY_NAME = "Goblinwatch"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract, export, and bundle only the original MM6 Goblinwatch indoor map for Unity testing."
    )
    parser.add_argument(
        "--install-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Destination folder for the bundled Goblinwatch Unity package.",
    )
    return parser.parse_args()


def extract_goblinwatch_map(install_dir: Path, generated_root: Path) -> Path:
...
def main() -> int:
    ...
    map_path = extract_goblinwatch_map(install.install_dir, generated_root)
